...
function composeAnswer(containerElt, answerData) {
  /*  
  ...
  if (answerData['spotted'] == true) {
    // Add text
    addDialogElt(containerElt, answerData['context'], 'answer');
    addDialogElt(containerElt, answerData['extract'], 'answer');
    // Add the address of the location
    addDialogElt(containerElt , `Voici où ton chemin te mène :   ${answerData['address']}`, 'answer');
    // Add a map of the location
    addMapElt(answerData);
  } else {
    // Add text
    addDialogElt(containerElt, answerData['context'], 'answer');
    addDialogElt(containerElt, answerData['reply'], 'answer');
  }
};
...
userForm.addEventListener('submit', function (event) {
  ...
  // Add an element to regroup the items
  let exchangeContainer = document.createElement('div');
  exchangeContainer.className = 'exchange';
  dialogBox.appendChild(exchangeContainer);


  // Add the question of the user to the main dialog
  addDialogElt(exchangeContainer, userText.value, 'question');
  ...
  postFormData("/ask", new FormData(userForm))
    .then(data => {
      ...
      composeAnswer(exchangeContainer, data)
    })
    .catch(error => console.log(error));
